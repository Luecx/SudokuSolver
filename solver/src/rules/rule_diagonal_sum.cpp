...
JSON RuleDiagonalSum::to_json() const {
    ...
    for (const auto &diagsum_pair: m_pairs) {
...
void RuleDiagonalSum::init_randomly() {
    ...
    const int num_pairs = diag_count_dist(gen);
    ...
    Region<DiagonalIdx> occupied_region; // use this so regions don't overlap

    int attempts = 0;
    while ((int) m_pairs.size() < num_pairs && attempts++ < 100) {
        const int region_size = region_size_dist(gen);
