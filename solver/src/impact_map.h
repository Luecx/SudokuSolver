...
public:
     ...
     */
    explicit ImpactMap(int size) : size_(size), impact_(size, std::vector<int>(size, 0)) {}
...
private:
    ...
    bool in_bounds(const CellIdx &idx) const { return idx.r >= 0 && idx.r < size_ && idx.c >= 0 && idx.c < size_; }
};
